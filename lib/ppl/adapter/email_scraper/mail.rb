...
#      display_name = from.tree.addresses.first.display_name
      display_name = from.display_names.first
      sender.name.full = Mail::Encodings.value_decode(display_name) unless display_name.nil?
#      sender.email_addresses << Ppl::Entity::EmailAddress.new(from.tree.addresses.first)
      sender.email_addresses << Ppl::Entity::EmailAddress.new(from.addresses.first)
